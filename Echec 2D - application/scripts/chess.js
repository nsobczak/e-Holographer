...
var chess = (function() {
    ...
    // Constellation
    constellation = $.signalR.createConstellationConsumer(
        "<URI du serveur>", "<clé d'acces au serveur>", "<friendly name>"),
	...
	constellation.connection.stateChanged(
	...
	*/
	...
	function createFlatCoord(nNewHeaderId, bVertOri) {
		...
		//constellation.server.sendMessage({ Scope: 'Package', Args: ['HolographerPythonPackage'] },
        //    'PlateauSuivant', '');
		//constellation.connection.start();  // On peut en mettre à différents endroit, ça fonctionnne
		return (oNewCoord);
	}
	...
	function showFlatBoard() {
		...
		// flat chessboard will be updated
		    updateFlatCoords();
		    console.log("updateFlatCoords dans showFlatBoard");
			if (!etc.bFlatView) {
		...
		// flat chessboard will be created
		    console.log("flat chessboard will be created dans showFlatBoard");
